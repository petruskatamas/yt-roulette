...
export const en = {
  ...
  join: {
    noLan: 'No network address found — this machine must be on the same Wi-Fi as the phones.',
    scanHint: 'Scan it with your phone → write your own bingo card. A tick marks a submitted card.',
  },

  host: {
    serverDown: 'Server unreachable — is `npm run dev` running?',
    offline: 'lost connection to the game server…',
    connect: 'Join',
    newGame: 'New game',
    players: 'Players',
    ...
    writingCard: 'writing card…',
    bingo: 'BINGO',
    cardOf: (name: string) => `${name}’s card`,
    stillWriting: 'Still writing their card…',
  },
